 ...
 */
@Component
public class ContractStateAssembler implements StateAssembler<ContractHeader> {
    ...
    @Autowired
    public ContractStateAssembler(ApplicationEventPublisher publisher) {
        this.publisher = publisher;
    }

    public StateConfig<ContractHeader> assemble() {
        StateBuilder<ContractHeader> builder = new StateBuilder<>();

        builder.beginWith(DRAFT).check(new ContentNotEmptyVerifier()).check(new AuthorIsNotAVerifier())
                .to(VERIFIED).action(new ChangeVerifier());

        builder.from(DRAFT).whenContentChanged()
                .to(DRAFT);
        ...
        builder.from(VERIFIED).check(new ContentNotEmptyVerifier())
                .to(PUBLISHED).action(new PublishEvent(DocumentPublished.class, publisher));

        builder.from(VERIFIED).whenContentChanged()
                .to(DRAFT);
